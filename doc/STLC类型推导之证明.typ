...
`Subst left right` 是一个依赖类型结构，将替换函数 `apply` 与三个不变量打包在一起。这三个不变量是终止性证明的关键：
...
$P_1$ 限制了像的变量域；$P_2$ 刻画了：若像的变量集合势与原集合相等，则该替换必为恒等替换（这是终止性的关键约束）；$P_3$ 是分配律。`Subst` 有四种构造子：`id`（恒等）、`replace`/`replacer`（单变量替换）、`comp`（复合）。
...
`unify` 的终止性并不显然，因为箭头情形进行了*两次*递归调用，且第二次调用作用于 $S_1$ 像——而非原始的子项。
...
#证明[
  ...
  *第二次调用* `unify(S₁(τ'), S₁(σ'))`：若势严格缩小则完成。否则势保持，由不变量 $P_2$ 推得 $S_1 = "id"$；当 $S_1 = "id"$ 时化简后尺寸严格缩小。
]
...
本章收录支撑主要可靠性定理的基础引理，均已在 Lean 4 中完全证明。
...
以下三个引理刻画了 `pp'` 的输出上下文与输入上下文之间的关系。
...
- *$t = lambda x . m$，$Gamma'(x) = "none"$*：`pp'` 分配 $alpha = "next"(Gamma')$，返回 $(Gamma'' + (x: alpha), alpha arrow.r tau)$。归纳假设加 `CheckInvUnderAdd`（新鲜性条件 $m eq.not "var" x$ 由"若 $m = "var" x$ 则 `check(Γ', var x) = none`，与归纳假设矛盾"推得）将结论提升至扩展上下文。

*应用情形*（此前为 sorry，现完整证明）。
...
#证明[
  $"pp"(t) = "pp'"(Gamma_emptyset, t)$。直接应用 `InferenceSound'`。在 `NextFresh` 假设下，定理无剩余 sorry。
]
...
#注记([`NextFresh`（标记为 sorry）])[
  上述证明用到了以下假设：`next ctx` 始终产生一个相对于 `ctx` *新鲜*的变量 $alpha$——即对 `ctx` 中任何现有变量 $x eq.not alpha$，向上下文中添加绑定 $(alpha, tau)$ 不影响对 $x$ 的查找：
  $
    "ctx"(x) = "some"(tau) arrow.l.r ("add"(y, "next"("ctx")."fst", "next"("ctx")."snd"))(x) = "some"(tau).
  $

  从实现来看，`next` 将 `label` 字符的序数加一，`add` 在环境前端插入新绑定，新鲜性在直觉上显然成立。然而，严格的形式化证明需要对 `Char.ofNat (ctx.label.toNat + 1)` 进行单射性论证——本质上涉及 `Nat → Char` 的有界单射性，包含字符编码的细节。该性质目前以 `sorry` 接受为公理。

  解决方案之一是将上下文中的类型变量改为用 `Nat` 而非 `Char` 索引，从而使新鲜性论证变为纯粹的自然数算术。
]

= 结论

本文给出了简单类型λ演算主类型推导算法的完整 Lean 4 形式化，包括：

+ STLC 语法、类型赋值规则与类型检查函数 `check`；
+ 带终止性证明的罗宾逊合一算法，利用依赖类型结构将终止性度量内嵌于 `Subst` 类型；
+ 上下文合一 `unifyCtx` 的正确实现（串行地传播替换，而非独立合一各对）；
+ 推导算法 `pp'` 关键结构性质的一套引理；
...
本文形式化的开发借助了 AI 语言模型（DeepSeek @deepseek）的辅助。 所有 AI 生成的证明草稿均经过仔细审查、修正与简化。
