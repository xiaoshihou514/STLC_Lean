...
#定义[
  ...
  表示函数 $f$ 在箭头类型上满足分配律。`CheckSubst` 等引理需要此条件来在类型替换下推进 `check`。
]
#定义[
...
#引理([上下文合一正确性])[
  ...
  `unifyCtx'` 的复合替换确实合一了输入列表中的每一对。证明对列表 $l$ 作归纳，利用 `UnifySound`（见第六章）和 `FoldlInv`。]<UnifyCtxSound>
#定理([公共变量类型一致])[
...
+ 推导算法 `pp'` 关键结构性质的一套引理；
+ 完整的正确性定理 `InferenceSound`，包括此前遗留的应用情形。
